...
{
    public partial class TK_XuatKho : Form
    {
        bool fl_new = true;
        DataTable dtInfo;
        DataTable dtItem;
        ...
        private void TK_XuatKho_Load(object sender, EventArgs e)
        {
            Common.obj.Add_NewCommand("Select Code = MaNX, Name = TenNX from TK_MaNX where LoaiNX = N'Xuat'", sys_ConData.sys_ConData.cmdType.Query);
            DataTable table = Common.obj.ExecCommand_ReturnTable(Common.obj.Get_NewCommand());
            cbo_LoaiNX.aaDataSource = table;
            cbo_LoaiNX.aaValueCode = "XH";
        ...
        private void cbo_LoaiNX_aaItemSelected(object sender, EventArgs e)
        {
            ...
            if (cbo_LoaiNX.aaValueCode.ToString() == "XH" | cbo_LoaiNX.aaValueCode.ToString() == "XHDC")
            {
                LoadCBO_KH(); 
            }
            else if (cbo_LoaiNX.aaValueCode.ToString() == "XHTL")
            {
                LoadCBO_NCC();
            }
        }
            ...
            if (dr == DialogResult.Yes)
            {
                ...
                if (txt_MaPhieu.Text != "")
                {
                    try
                    {
                        ...
                        Common.obj.Add_NewCommand("Update TK_DonHangXuat set Huy = 1 where MaPhieu = @MaPhieu", sys_ConData.sys_ConData.cmdType.Query);
                        Common.obj.Add_MyParametersCommand("@MaPhieu", SqlDbType.NVarChar, txt_MaPhieu.Text);
            ...
            else
            {
                if (txt_MaPhieu.Text != "")
                {
                    if (g_Item.Rows.Count > 0)
                    {
                        ...
                        Common.obj.BeginTransaction();
                        Common.obj.Add_NewCommand("Update TK_DonHangXuat set NgayLap = @NgayLap, MaKH = @MaKH, NgayGiao = @NgayGiao, MaKho = @MaKho, LoaiNX = @LoaiNX where MaPhieu = @MaPhieu", sys_ConData.sys_ConData.cmdType.Query);
                        Common.obj.Add_MyParametersCommand("@MaPhieu", SqlDbType.NVarChar, txt_MaPhieu.Text);
                        Common.obj.Add_MyParametersCommand("@MaKH", SqlDbType.NVarChar, cbo_KH.aaValueCode.ToString());
                        Common.obj.Add_MyParametersCommand("@LoaiNX", SqlDbType.NVarChar, cbo_LoaiNX.aaValueCode.ToString());
                        Common.obj.Add_MyParametersCommand("@MaKho", SqlDbType.NVarChar, cbo_Kho.aaValueCode.ToString());
                        ...
                        Common.obj.Add_MyParametersCommand("@NgayGiao", SqlDbType.NVarChar, dtp_NgayGiao.Value.ToString());
                        Common.obj.ExecCommand_NoReturn(Common.obj.Get_NewCommand());
                        Common.obj.Add_NewCommand("Delete TK_DonHangXuat_Item where MaPhieu = @MaPhieu", sys_ConData.sys_ConData.cmdType.Query);
                        Common.obj.Add_MyParametersCommand("@MaPhieu", SqlDbType.NVarChar, txt_MaPhieu.Text);
                        Common.obj.ExecCommand_NoReturn(Common.obj.Get_NewCommand());
                        Common.obj.Add_NewCommand("Select MaHang, SL, DonGia, ThanhTien, GhiChu, MaPhieu from TK_DonHangXuat_Item", sys_ConData.sys_ConData.cmdType.Query);
                        Common.obj.UpdateTable(Common.obj.Get_NewCommand(), table2);
        ...
        {
            Common.obj.Add_NewCommand("BC_BK_DonHangXuat", sys_ConData.sys_ConData.cmdType.Store);
        ...
        private void g_BC_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (g_BC.Rows.Count > 1)
            {
                if (g_BC[g_BC.Row, "MaPhieu"].ToString() != "")
                {
                    ...
                    Common.obj.Add_NewCommand("Select * from TK_DonHangXuat where MaPhieu = @MaPhieu", sys_ConData.sys_ConData.cmdType.Query);
                    Common.obj.Add_MyParametersCommand("@MaPhieu", SqlDbType.NVarChar, txt_MaPhieu.Text);
                    DataTable table = Common.obj.ExecCommand_ReturnTable(Common.obj.Get_NewCommand());
